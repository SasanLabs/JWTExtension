...
now say server is expecting to verify the algorithm using RSA or more specifically if server is only encrypting the token with RSA and say hacker has signed the token with Public key and set the Algorithm as "HS" ie HMAC with SHA then while calling verify method, server will send token and Public Key (Why ? because server only supports RSA) but the library will think HS as the algorithm so it will think verificationKey as HMAC secrete key and will return it as a valid.
```So Solution is that verification method to accept algorithm too.```
but there can be a counter argument that if server is allowing multiple algorithms so server might not know which algorithm to pass to the library then how we can handle this usecase?
